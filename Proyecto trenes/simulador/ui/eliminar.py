...
def eliminar_estacion(ventanafaz):  
    ventana = tk.Toplevel(ventanafaz)  
    ventana.title("Eliminar estacion")  
    ventana.geometry("300x300")  
    tk.Label(ventana, text="Eliminar estación").pack(pady = 5)  
    if not estaciones_guardadas:  
        tk.Label(ventana, text="No existe esta estación").pack(pady =  10)  
        return  
    estacion_puntito = tk.StringVar(ventana)  
    #mostrar el nombre
    for e in estaciones_guardadas:
        display = e["nombre"] if isinstance(e, dict) else str(e)
        tk.Radiobutton(ventana, text = display, variable =estacion_puntito, value= display).pack(anchor = "w")  

    def confirmar():  
        seleccionado = estacion_puntito.get()
        # elimina diccionario
        for est in list(estaciones_guardadas):  # iteramos sobre copia para remover
            if isinstance(est, dict) and est.get("nombre") == seleccionado:
                estaciones_guardadas.remove(est)
                break
    ...
    def confirmar():  
        ...
        for item in list(trenes_guardados):  # iteramos sobre copia para poder remover
            if isinstance(item, dict) and item.get("nombre")==seleccionado:
                ...
                trenes_guardados.remove(item)
                break
        print("Trenes actulez:", trenes_guardados)  
        ventana.destroy()
    tk.Button(ventana, text = "Eliminar", command = confirmar).pack(pady = 10)